...
/*  slist[0-1]   NOT USED
 *  slist[2-64]  ----   2*8(16) ~ 64*8(512)
 *  slist[65]    ----   512~1024
 *  slist[66]    ----   1024~2048
 *  slist[67]    ----   2048~4096
 *  slist[68]    ----   >4096
 */
static char *slist[69];
...
/*
    char *node;
    for (node = NEXT_FREEB(head); node != head; node = NEXT_FREEB(node)) {
	if ((unsigned int)node > (unsigned int)bp) {
	    insert_after(PREV_FREEB(node), bp);
	    return;
	}
    }
    insert_after(PREV_FREEB(head), bp);
*/
/* 
    insert_after(head, bp);
*/
}
...
static inline int get_index(size_t size)
{
    ...
    else if (size <= 1024)   return 65;
    else if (size <= 2048)   return 66;
    else if (size <= 4096)   return 67;
    else                     return 68;
}
